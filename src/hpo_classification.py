...
OBJECTIVES = [
                "MASE_Imputation", 
                "Scaled_RMSE_Imputation", 
                "MASE_Forecast", 
                "Scaled_RMSE_Forecast"
              ]
PREFERENCE_FUNCTION = {
    "MASE_Imputation": 1, 
    "Scaled_RMSE_Imputation": 1,
    "MASE_Forecast": 1, 
    "Scaled_RMSE_Forecast": 1,
}
...
def main(
    ...
    checkpointing: str = Option("none", help="Which Checkpointing Mode to use 'official' for save behaviour, 'none' to start from scratch"),
):
...
def train(
    ...
    try:
        ...
        for p in training_data_paths_list:
            dataset_name = Path(p).stem
            if dataset_name in config:
                config.pop(dataset_name)

        probability = config.pop("probability")
        probabilities = literal_eval(probability)
        ...
        try:
            ...
            val_config = Path("./data/eval_configs/in-domain.yaml").resolve()
