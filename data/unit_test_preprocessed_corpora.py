...
if __name__ == "__main__":
    ...
    # TODO uci_corpus = ds.load_from_disk(data_directory / "UCI_Corpus_Processed")["train"]
    print(kaggle_corpus)
    #TODO print(uci_corpus)

    corpora = [("Kaggle", kaggle_corpus)] # TODO ("UCI", uci_corpus),
    ...
    #TODO assert 2_500 > len(uci_corpus) >= 2_200, f"Only {len(uci_corpus)} of 2_328 original TS in UCI Corpus some seem to have gone missing, small changes are expected though as data is not completly stable"
    ...
    for name, corpus in corpora:
        ...
        assert total_length > 25.5e6 # TODO If kaggle else ... # 25_749_461 (Kaggle)
        avg_length = total_length / len(lengths)
        print(f"Avg Length: {avg_length}")
        assert avg_length > 1200 # TODO If kaggle else ...
