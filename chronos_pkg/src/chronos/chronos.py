...
class ChronosConfig:
    ...
    eos_token_id: int
    use_eos_token: bool
    model_type: Literal["causal", "seq2seq", "mlm"]
    num_samples: int
    temperature: float
    top_k: int
    top_p: float
    ...
    def __post_init__(self):
        ...
        if self.mask_token_id is not None:
            assert self.mask_token_id < self.n_special_tokens, \
                f"mask_token_id must be < n_special_tokens ({self.n_special_tokens})"
...
class MeanScaleUniformBins(ChronosTokenizer):
    ...
    def output_transform(self, samples: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
        ...
        centers = self.centers.to(indices.device)  # <-- hier fix
...
class ChronosModel(nn.Module):
    ...
    def forward(
        ...
        """
        Forward pass for Chronos models.

        - MLM: supervised masked-token reconstruction (returns loss + logits)
        - Causal/Seq2Seq: autoregressive sample generation
        """
        # === Masked LM path ===
        if self.config.model_type == "mlm":
            outputs = self.model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                labels=labels,
            )
            # HuggingFace MLM models return {"loss", "logits", ...}
            return outputs
...
class ChronosPipeline(BaseChronosPipeline):
    ...
    def impute_span(
        self,
        series: Union[torch.Tensor, List[torch.Tensor]],
        ...
        series_tensor = self._prepare_and_validate_context(series=series)
        ...
        # get mask token
        mask_token_id = getattr(self.tokenizer.config, "mask_token_id", None)
        if mask_token_id is None:
            raise ValueError("Tokenizer must define `mask_token_id` for MLM-style imputation.")

        # valid (non-special) positions
        special_token_cutoff = self.tokenizer.config.n_special_tokens
        valid_positions = input_ids >= special_token_cutoff
        ...
        sampled_ids = sampled_token_ids.unsqueeze(0)

        decoded = self.tokenizer.output_transform(
                sampled_ids.to(scale.device), scale
            )

        return decoded.to(dtype=torch.float32), mask_positions
    

    @classmethod
    def from_pretrained(cls, *args, task="forecasting", num_labels=6, **kwargs):
        """
        ...
        elif chronos_config.model_type == "mlm":
